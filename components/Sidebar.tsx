...
export default function Sidebar() {
  ...
  const q = mes ? `?mes=${mes}` : "";
  return (
    <aside className="border-line bg-[#0A1526] lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-[218px] lg:shrink-0 lg:flex-col lg:border-r">
          ...
          return (
            <Link
              ...
              className={`relative flex shrink-0 items-center gap-3 whitespace-nowrap rounded-lg px-4 py-2.5 text-[15px] lg:mx-2 lg:py-3 ${
                ativo ? "bg-verde/15 font-semibold text-ink" : "text-mute hover:text-ink"
              }`}
            >
              {ativo && <span className="absolute left-[-8px] top-1 bottom-1 hidden w-1 rounded-r bg-verde lg:block" />}
              <I className={`h-5 w-5 shrink-0 ${ativo ? "text-verde" : ""}`} />
              {nome}
            </Link>
          );
        })}
      ...
      <div className="mt-auto hidden px-7 pb-10 lg:block">
        ...
        <div className="mt-5 h-0.5 w-10 bg-verde" />
      </div>
    </aside>
  );
}
